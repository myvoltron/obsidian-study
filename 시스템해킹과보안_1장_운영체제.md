...
>> # Windows 부팅 순서
>> ## 공통 Steps
>> 1. POST(Power On Self Test) 실행 - BIOS(Basic Input/Output system)이 POST를 실행. 하드웨어 스스로 문제가 없는지 기본 사항 점검.
>> 2. 기본 부팅 관련 설정 로드
>> 	- BIOS가 CMOS에 설정된 시스템 설정 사항 및 부팅과 관련된 정보를 읽어 시스템에 적용.
>> 	- CMOS(Complementary Metal-Oxide Semiconductor
>> 3. MBR(Master Boot Record) 로드
>> 	- MBR은 저장 매체의 첫 번째 섹터인 LBA 0에 위치한 512 바이트 영역으로, 부팅 매체의 파일 시스템 정보를 가지고 있다.
>> 	- ***Missing operating system*** 메시지는 OS 미설치 혹은 부팅 미디어 오설정
>> ### Windows XP, Windows Server 2000/2003 부팅 순서
>> 4. NTLDR(NT Loader) 실행
>> 	- 하드디스크의 부팅 파티션에 있는 프로그램이다. 윈도우 부팅을 위해 간단한 파일 시스템을 실행하고 `boot.ini` 파일을 읽어 부팅 옵션들을 보여준다
>> 5. NTDETECT.com 실행
>> 	- NTDETECT는 NTLDR에서 제어권을 위임받아 하드웨어 검사.
>> 	- ex) CPU type, bus type, video board type, a kind of keyboard and mouse . . 
>> 6. ntoskrnl.exe(NT OS Kernel) 실행 - HAL.DLL. 로드
>> 	a) Load Kernel : Windows Server 2000 loads system settings and stores these in HKEY_LOCAL_MACHINE \System\CurrentControlset\Services. It checks these information and determines a loading drive and sequences
>> 	b) Kernel initialization : Checks current control setting on a drive and starts a job
>> 	c) Load services : Load Session Manager Subsystem(smss.exe) and Win32 subsystem
>> 	d) Start subsystem : Windows subsystem is initialized. Win32 subsystem processes login and starts Winlogon.exe. Login window is activated when Ctrl+Alt+Del are pressed and takes account and password input sent to Local Security Authentication Server (lsass.exe). Local Security Authentication Server (lsass.exe) receiving account and password compares it with stored information in Security Accounts Manager (SAM). If they match, Userinit.exe process executes a shell referenced by HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Winlogon
>> ### The boot sequence of Windows Vista, 2008, 7, 10, (11)
>> 7. Window Boot Manager 실행
>> 	- `bootmgr.exe` 를 실행하고 Boot Configuration Data(BCD)를 읽어서 실행 가능한 OS 목록을 보여준다. `bcdedit.exe`를 통해서 편집 가능
>> 8. Windows OS loader(Winload.exe) 실행
>> 	- Winload(Windows OS loader) 각종 장치 드라이버 로드, `ntoskrnl.exe` 실행
>
>> [!note]+ Linux
...
>> - Pipe file : `|` 문자를 말함. 명령 2개를 연결할 때 사용.ㄹ
>> # The boot sequence of Linux/Unix
>> step 1 ~ 3은 윈도우와 비슷함.
>> 1. Boot Loader 실행
...
>> 1. 실행 레벨에 따른 서비스 실행
>> 	- 부트 로더는 pid 0 process인 Swapper를 실행하고 그 Swapper는 pid1 init(`/sbin/init`) process를 실행함.
>> 	- init process는 `/etc/inittab` 파일을 읽음
>> 	- inittab 파일은 부팅할 기본 모드를 선택하여 그에 따라 환경을 제공하는 분기점. 프로그램 종류에 따라 다른 수준의 실행 레벨을 제공함. (default 5)
>> 		- Run level 0 : 시스템 종료 시 사용됨
>> 		- Run level 1 : 단일 사용자 모드, 관리자 권한의 셸이지만 대부분 데몬을 실행하지 않아 기능 제약.
>> 		- Run level 2 : 다중 사용자 모드 without NFS
>> 		- Run level 3 : 다중 사용자 모드 with 셸 기반의 인터페이스
>> 		- Run level 4 : 기본적으로 사용하지 않지만 임의로 정의하여 사용할 수 있음

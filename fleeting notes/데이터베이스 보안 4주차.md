...
# 1장. 구조화 질의어(SQL) 개념
## 1.1 SQL 개념
- Structured Query Language: 관계형 데이터베이스 관리 시스템의 데이터를 정의하고 조작하기 위한 표준 언어
- 데이터 검색, 삽입, 수정, 삭제, 병행 제어 등을 지원하는 데이터베이스 전용 언어
...
### 2.1.6 제약조건
- **개체 무결성 제약조건**: 기본키 → NULL 불허, 유일값
- **참조 무결성 제약조건**: 외래키는 참조되는 기본키와 일치해야 함
- **도메인 무결성 제약조건**: CHECK, 데이터 타입 등으로 값 제약
## 2.2 ALTER 문
- 기존 테이블 구조를 수정할 때 사용
...
# 3장. 데이터 조작어(DML)
## 3.1 데이터 조작어 개념
- 데이터베이스에 저장된 데이터를 조회하거나 변경할 수 있는 언어
- SELECT, INSERT, UPDATE, DELETE 포함
## 3.2 SELECT/FROM 문
- 테이블에서 데이터를 조회할 때 사용
...
- `INSERT INTO 테이블명 SELECT ... FROM 다른 테이블 WHERE 조건;`
## 3.7 UPDATE 문
- 테이블의 기존 값을 수정
...
### 4.1.2 시스템 권한 부여
- `GRANT CREATE TABLE TO 사용자;` (DBA만 가능)
- ROLE 생성 후 ROLE에 권한 부여 가능
## 4.2 REVOKE 문
### 4.2.1 기본 구조
...
## 4.3 기타
- ROLLBACK, COMMIT 등 트랜잭션 제어 문도 포함되나 이번 강의에서는 다루지 않음

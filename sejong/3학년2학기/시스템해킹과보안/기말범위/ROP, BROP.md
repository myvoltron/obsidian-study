...
## 1.1 ROP 개요 및 정의
- 공격자가 NXbit 및 코드 서명과 같은 보안이 활성화된 상태에서 코드를 실행할 수 있도록 해주는 기술.
- RTL(Return-to-libc) + Gadget + Stack Overflow
- 공격자는 실제 함수가 필요하지 않으며, 대신 프로그램의 메모리 공간에 이미 존재하는 명령어 시퀀스를 활용하여 임의의 동작을 수행합니다.
